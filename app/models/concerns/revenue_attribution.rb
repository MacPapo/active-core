module RevenueAttribution
  extend ActiveSupport::Concern

  included do
    scope :revenue_generating, -> {
      joins(:payment).where(payments: { income: true })
    }
    scope :this_month_revenue, -> {
      revenue_generating.joins(:payment)
        .where(payments: { date: Date.current.beginning_of_month.. })
    }
  end
