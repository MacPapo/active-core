module User::ActivityTracking
  extend ActiveSupport::Concern

  included do
    scope :active_today, -> { where(current_sign_in_at: Date.current.all_day) }
    scope :by_activity_period, ->(from, to) { where(current_sign_in_at: from..to) if from && to }
  end
