module Pricing::Integration
  extend ActiveSupport::Concern

  included do
    scope :with_active_pricing, -> { joins(:pricing_plans).where(pricing_plans: { active: true }).distinct }
    scope :by_price_range, ->(min, max) { joins(:pricing_plans).where(pricing_plans: { price: min..max }).distinct }
  end
